   ...
   , m_ScaleFactorTool( this )
{
  ...
  DeclareProperty( "TauPtCut",             m_tauPtCut              = 20   ); //30
...
//   DeclareProperty( "JSONName",             m_jsonName              = std::string (std::getenv("SFRAME_DIR")) + "/../GoodRunsLists/JSON/Cert_271036-280385_13TeV_PromptReco_Collisions16_JSON_NoL1T_v2.txt" );
  DeclareProperty( "JSONName",             m_jsonName              = std::string (std::getenv("SFRAME_DIR")) + "/../GoodRunsLists/JSON/Cert_271036-276811_13TeV_PromptReco_Collisions16_JSON.txt" );
...
//   std::cout << "FillBranches" << std::endl;
  ...
  //  else          b_npu[ch]   = (*m_eventInfo.averageIntPerXing)[0]; //actualIntPerXing)[0];
...
//    
//    // Di-muon veto
//    if( !b_dilepton_veto_     && mymuon.pt() > 15       &&
//        mymuon.isGlobalMuon() && mymuon.isTrackerMuon() && mymuon.isPFMuon()){
//      for( int j = 0; j < (int)passedMuons.size(); ++j ){
//        if(mymuon.charge() * passedMuons[j].charge() > 0) continue;
//        if(mymuon.tlv().DeltaR(passedMuons[j].tlv()) < 0.15) continue;
...
//  }
  ...
  for( int i = 0; i < m_muon.N; ++i ){
    ...
    if(mymuon.isMediumMuon() > 0.5){
      if(channel=="mutau"){
	if(mymuon.eta() != muon.eta() && mymuon.pt() != muon.pt() && mymuon.phi() != muon.phi()) b_extramuon_veto_ = true;
      }else{
	b_extramuon_veto_ = true;
      }
    }
...
//      }
  ...
  for( int i = 0; i < m_electron.N; ++i ){
    ...
    if(myelectron.passConversionVeto() &&
      ...
      if(channel=="eletau"){
	if( myelectron.pt() != electron.pt() && myelectron.eta() != electron.eta() && myelectron.phi() != electron.phi()) continue;
      }else{
	b_extraelec_veto_ = true;
      }
    }
