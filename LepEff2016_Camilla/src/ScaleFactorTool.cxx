 ...
 SToolBase( parent ), m_name( name ) 
{
  ...
  DeclareProperty( m_name+"EleTrigName",    m_Ele_TrigFile      = std::string (std::getenv("SFRAME_DIR")) + "/../LepEff2016/data/Electron/Electron_Ele25eta2p1WPTight_eff.root" );
  DeclareProperty( m_name+"EleIDIsoSFName", m_Ele_IDIsoSFFile   = std::string (std::getenv("SFRAME_DIR")) + "/../LepEff2016/data/Electron/Electron_IdIso0p10_eff.root" );
...
void ScaleFactorTool::BeginInputData( const SInputData& ) throw( SError ) {
  ...
  init_ScaleFactor( m_Ele_IDIsoSFFile );
  m_logger << INFO <<"Scale factor Ele IDIso initialised"<< SLogger:: endmsg;

  init_ScaleFactor( m_Ele_TrigFile );
...
void ScaleFactorTool::init_ScaleFactor(TString inputRootFile){
  ...
  if (inputRootFile.Contains("egamma")){
  ...
  else  if (inputRootFile.Contains("Electron_Ele")){
    auto fileIn = TFile::Open(inputRootFile// .c_str()
			      );
    if (fileIn->IsZombie() ) {
      if (verbose)std::cout << "ERROR in ScaleFactor::init_ScaleFactor(TString inputRootFile) from NTupleMaker/src/ScaleFactor.cc : ‎File " <<inputRootFile << " does not exist. Please check. " <<std::endl; 
    ...
    etaptBinsH_TrigEle = (TH2D*)fileIn->Get("IsoMu22_OR_IsoTkMu22_PtEtaBins_Run274094_to_276097/efficienciesDATA/pt_abseta_DATA"); 
    etaptBinsH_TrigEle->SetDirectory(0);
    fileIn->Close();

  }
  else  if (inputRootFile.Contains("Electron_IdIso")){
    auto fileIn = TFile::Open(inputRootFile// .c_str()
			      );
    if (fileIn->IsZombie() ) {
      if (verbose)std::cout << "ERROR in ScaleFactor::init_ScaleFactor(TString inputRootFile) from NTupleMaker/src/ScaleFactor.cc : ‎File " <<inputRootFile << " does not exist. Please check. " <<std::endl; 
    ...
    etaptBinsH_IDIsoEle = (TH2D*)fileIn->Get("IsoMu22_OR_IsoTkMu22_PtEtaBins_Run274094_to_276097/efficienciesDATA/pt_abseta_DATA"); 
    etaptBinsH_IDIsoEle->SetDirectory(0);
    fileIn->Close();
